...
def compute_portvals(orders_file = "./orders/orders.csv", start_val = 1000000, commission=9.95, impact=0.005): 			  		 			     			  	   		   	  			  	
    ...
    if type(orders_file) == str:
        ...
        prices = get_data([trades.columns[0]], pd.date_range(start_date, end_date))
        prices.ffill(inplace=True)
        prices.bfill(inplace=True)
        prices['CASH'] = 1.0
        trades['CASH'] = 0.0
        ...
        for index, row in trades.iterrows():
            if row[trades.columns[0]] > 0:
                trades.loc[index, 'CASH'] = (-1*(1+impact)*row[trades.columns[0]]*prices.loc[index, trades.columns[0]]) - commission
            elif row[trades.columns[0]] < 0:
                trades.loc[index, 'CASH'] = (-1*(1-impact)*row[trades.columns[0]]*prices.loc[index, trades.columns[0]]) - commission
            else:
                pass   
